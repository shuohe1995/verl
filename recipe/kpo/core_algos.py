...
def compute_policy_loss_gspo(
    ...
    pg_metrics = {
        "actor/pg_clipfrac": pg_clipfrac.detach().item(),
        "actor/ppo_kl": ppo_kl.detach().item(),
        "actor/sequence_ratio": seq_importance_ratio.detach().mean().item(),
        "actor/pg_clipfrac_lower": pg_clipfrac_lower.detach().item(),
    }

    debug_metrics = {   
        "token_ratio": torch.exp(negative_approx_kl).detach(),
        "sequence_ratio": seq_importance_ratio.detach().mean().item(),
...
def generate_test_data(batch_size, seq_len, mask_type='ones'):
    """
    Generate full data for testing.

    Args:
        batch_size: batch size
        seq_len: sequence length
        mask_type: 'ones' (all 1s) or 'partial' (partially 1s)

    Returns:
        old_log_prob, log_prob, advantages, response_mask
    """
    # Generate log probs (simulate old/new policy)
    torch.manual_seed(40)  # fix seed for reproducibility

    # Old policy probs in [0, 1], then log
    old_prob = torch.rand(batch_size, seq_len)  # [0, 1]
    old_log_prob = torch.log(old_prob + 1e-8)  # add small value to avoid log(0)

    # New policy probs in [0, 1], then log
    new_prob = torch.rand(batch_size, seq_len)  # [0, 1]
    log_prob = torch.log(new_prob + 1e-8)  # add small value to avoid log(0)

    # Generate advantages
    advantages = torch.randn(batch_size, seq_len) * 0.8  # mean 0, std 0.8

    # Generate mask
    if mask_type == 'ones':
        response_mask = torch.ones(batch_size, seq_len, dtype=torch.float32)
    else:
        # Partial mask; padding usually at sequence end
        response_mask = torch.ones(batch_size, seq_len, dtype=torch.float32)
        # Randomly zero some positions (simulate padding)
        for i in range(batch_size):
            pad_start = torch.randint(int(seq_len * 0.7), seq_len, (1,)).item()
            response_mask[i, pad_start:] = 0
    
    return old_log_prob, log_prob, advantages, response_mask

def test_compute_policy_loss():
    b, t = 1, 100
    old_log_prob, log_prob, advantages, response_mask = generate_test_data(
        batch_size=b, 
        seq_len=t, 
        mask_type='ones'
    )
    ...
    class MockConfig:
        ...
        global_batch_info = {"global_batch_size": b}

    config = MockConfig()

    _, dbg1 = compute_policy_loss_kpo(
    ...
    _, dbg3 = compute_policy_loss_gspo(
        old_log_prob=old_log_prob,
        log_prob=log_prob,
        advantages=advantages,
        response_mask=response_mask,
    ...
    tlr = dbg1["token_ratio"][0]
    fal = dbg1["smoothed_token_ratio"][0]
    srm = dbg1["sequence_ratio_mean"]
    pg_clipfrac = dbg1["pg_clipfrac"]   
    # clip_scale = dbg1["clip_scale"]
    # lower_bound = dbg1["lower_bound"]
    # upper_bound = dbg1["upper_bound"]

    # for x,y in zip(tlr, fal):
    #     print(f"{x:.8f}-{y:.8f}")
    print(f"token_ratio={tlr}")
    print(f"smoothed_token_ratio={fal}")

    print(f"sequence_ratio_mean={srm} | pg_clipfrac={pg_clipfrac}")
    #print(f"clip_scale={clip_scale} | ")
    # print(f"lower_bound={lower_bound} | ")
    # print(f"upper_bound={upper_bound} | ")

    print("=============gspo===============") 

    pg_clipfrac = dbg3["pg_clipfrac"]
    fal = dbg3["sequence_ratio"]

    print(f"sequence_ratio={fal} | pg_clipfrac={pg_clipfrac}")
